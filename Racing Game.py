...
BGSPEED = 1.5
BGIMG = pygame.image.load('img/background-1.png')
BGIMG = pygame.transform.scale(BGIMG,(400,1000))
...
CARHEIGHT = 60
CARSPEED = 3
CARIMG = pygame.image.load('img/car2.png')
...
OBSTACLESSPEED = 2
CHANGESPEED = 0.001
OBSTACLESIMG = pygame.image.load('img/YellowBuggy_0.png')
OBSTACLESIMG = pygame.transform.scale(OBSTACLESIMG,(CARWIDTH,CARHEIGHT))
...
def isGameover(car, obstacles):
    ...
    for i in range(5):
        ...
        B = [x,y]
        if collision(CARIMG,A ,OBSTACLESIMG,B) == True:
            return True
    return False
...
def gameOver(bg, car, obs, score):
    ...
    scoreSuface = font.render('Score: '+str(int(sc)), True, (0, 0, 0))
    scoreSufaceSize = scoreSuface.get_size()
    x1 = (WINDOWWIDTH-scoreSufaceSize[0])/2
    x2 = (WINDOWHEIGHT-scoreSufaceSize[1])/2
    ...
    while True:
        ...
        car.draw()
        obs.draw()
        DISPLAYSURF.blit(scoreSuface, (x1,x2))
        DISPLAYSURF.blit(headingSuface, (int((WINDOWWIDTH - headingSize[0])/2), 100))
        DISPLAYSURF.blit(commentSuface, (int((WINDOWWIDTH - commentSize[0])/2), 400))
        pygame.display.update()
        fpsClock.tick(FPS)
...
def main():
    ...
    gameStart(bg)
    while True:
        gamePlay(bg,car,obs,score)
        gameOver(bg, car, obs, score)
